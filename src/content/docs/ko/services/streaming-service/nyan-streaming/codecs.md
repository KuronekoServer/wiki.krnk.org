...
Nyan Streaming은 자체 개발한 송출 서버에서 동작하며 영상은 **서버 측에서 트랜스코딩(재인코딩)하지 않는** 구조입니다. 음성은 AAC가 표준이며, RTSP로 입력된 LPCM을 RTSP 이외의 방식으로 시청할 때만 AAC로 변환합니다. **지원 코덱으로 송출해야 하며**, 지원하지 않는 코덱을 사용하면 영상이나 음성이 나오지 않을 수 있습니다.
...
- **LPCM(pcm_s16le / 무압축·최고 음질)** — LPCM 송출은 **RTSP 푸시에서만** 이용할 수 있습니다. RTSP 시청에서는 LPCM / L16(PCM)으로 받을 수 있지만, HLS / RTMP / WebRTC 등 **RTSP 이외의 방식으로 시청하면 음성은 AAC로 제공됩니다**.
...
- 영상: **H.264**
- 음성: **AAC(MPEG-4 Audio)**, **LPCM(pcm_s16le, RTSP 시청 시에만)**
  :::
