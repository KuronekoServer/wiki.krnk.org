...
特定の単語を好みの読みに置き換えたいときに使います。

## サブコマンド一覧
コマンド名 | 概要 | 補足
-- | -- | --
/dictionary help | 辞書コマンドの一覧を表示します。 | サーバー内で使える辞書操作を確認できます。
/dictionary add | 辞書に単語を登録します。 | 正規表現は上位プラン向けです。
/dictionary remove | 辞書から単語を削除します。 | 単語名はオートコンプリートに対応しています。
/dictionary export | 辞書をエクスポートします。 | csv, json, colon に対応しています。
/dictionary import | 辞書をインポートします。 | 1MB以下のJSON形式での再取り込みを推奨します。
/dictionary list | 辞書の一覧を表示します。 | ページ送りで確認できます。
...
補足:
- 登録件数の上限と、単語・読みの文字数の上限はプランにより変わります。
- 正規表現を使った登録には Advance プラン以上が必要です。
- 同じ単語を登録すると上書きされます。
...
## 関連ページ
- [/user-dictionary](/services/discordbot/tts-bot/user-dictionary/)
