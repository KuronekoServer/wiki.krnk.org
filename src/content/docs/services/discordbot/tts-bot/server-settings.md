...
/server-settings speed | サーバー標準のスピードを設定します。 | 0.5から4.0まで指定でき、未指定でリセットされます。
/server-settings pitch | サーバー標準のピッチを設定します。 | BOT設定の許容範囲で指定でき、未指定でリセットされます。
/server-settings intonation | サーバー標準のイントネーションを設定します。 | 0.0から2.0まで指定でき、未指定でリセットされます。
/server-settings volume | サーバー標準の音量を設定します。 | 0.1から5.0まで指定でき、未指定でリセットされます。
/server-settings force-guild-args | サーバー標準の速度、ピッチ、イントネーション、音量を強制します。 | 個人設定より優先されます。
/server-settings force-guild-speaker | サーバー標準の話者を強制します。 | 個人の話者設定より優先されます。

## 話者制御
コマンド名 | 概要 | 補足
-- | -- | --
/server-settings disable-speaker add | 特定の話者を無効化します。 | デフォルト話者とサーバー標準話者は無効化できません。
/server-settings disable-speaker remove | 無効化した話者を再び有効化します。 | 話者名はオートコンプリートに対応しています。
...
/server-settings length-limit | 1メッセージあたりの最大読み上げ文字数を変更します。 | 30文字より多く、かつプラン上限以内で指定します。
...
/server-settings exclude-user add | 特定のユーザーを読み上げ対象から除外します。 | 登録件数の上限はプランによって変わります。
/server-settings exclude-user remove | ユーザーの除外を解除します。 | Botは対象外です。
/server-settings exclude-user list | 除外中のユーザー一覧を表示します。 | 一覧表示のみです。
/server-settings exclude-role add | 特定のロールを持つユーザーを読み上げ対象から除外します。 | 登録件数の上限はプランによって変わります。
/server-settings exclude-role remove | ロールの除外を解除します。 | @everyone を指定すると全ロールの除外が解除されます。
...
/server-settings autojoin list | 登録済みの自動接続設定を表示します。 | VCを指定すると、そのVCだけ確認できます。
...
- 詳細は [有料プランについて](/services/discordbot/tts-bot/payment/) を参照してください。

## 関連ページ
- [/subscription](/services/discordbot/tts-bot/subscription/)
- [有料プランについて](/services/discordbot/tts-bot/payment/)
