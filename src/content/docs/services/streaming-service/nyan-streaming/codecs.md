...
Nyan Streaming は自社開発の配信サーバーで動作しており、映像は**サーバー側でトランスコード（再エンコード）を行わない**設計です。音声は AAC を標準とし、RTSP で入力された LPCM を RTSP 以外の方式で視聴する場合のみ AAC に変換します。**対応コーデックで配信していただくことが必須**であり、非対応のコーデックでは映像・音声が表示されない・聞こえないなどの問題が発生します。
...
- **LPCM（pcm_s16le / 非圧縮・最高音質）** — LPCM で配信できるのは **RTSP プッシュのみ**です。RTSP で視聴する場合は LPCM / L16（PCM）で受け取れますが、HLS / RTMP / WebRTC など **RTSP 以外の方式で視聴する場合、音声は AAC になります**。
