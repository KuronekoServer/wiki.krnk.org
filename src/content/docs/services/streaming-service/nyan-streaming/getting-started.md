...
サーバーは映像を再エンコードしません。非対応の映像・音声コーデックを選ぶと、配信を開始できても視聴側で再生されない場合があります。対応音声は AAC と PCM（s16be）です。**PCM（s16be）で配信できるのは RTSP のみで、RTSP 以外の方式で視聴する場合は AAC になります。** 設定済みプロファイルは [低遅延 OBS プロファイル](/services/streaming-service/nyan-streaming/obs-profile/) で配布しています。
