...
Nyan Streaming は、**高画質な映像を低遅延（最短で約 1 秒）で配信すること**を目標とした映像配信サービスです。自社開発の配信サーバーで動作しており、映像はサーバー側で**トランスコード（再エンコード）を行わず**に視聴者へ届けます。RTSP で入力された PCM（s16be）音声を RTSP 以外で視聴する場合のみ AAC に変換します。
...
映像は **H.264**、音声は **AAC** と **PCM（s16be・RTSP）** に対応しています。WebRTC では Opus も利用できます。PCM（s16be）配信を RTSP 以外で視聴する場合、音声は AAC になります。**AV1 / VP9 / VP8 / H.265 (HEVC) には対応していません。** 詳細は [対応コーデック一覧](/services/streaming-service/nyan-streaming/codecs/) をご覧ください。
...
- **音声 EQ・リバーブ** — 配信ごとにサーバー側でイコライザーとリバーブを個別に適用できます。
